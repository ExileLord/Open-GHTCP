...
namespace GHNamespace2
{
    ...
    {
        [FieldOffset(0)] private int int_0;
        ...
        private int method_9(string string0, int int2, int int3)
        {
            ...
            if ((num & 3) != 0)
            {
                throw new ArgumentOutOfRangeException(string0,
                    string.Format("{0} cannot set a count ({1}) that is not 4 bytes aligned ", string0, num));
            }
            if (int2 < 0 || int2 > byte_0.Length >> int3)
            {
                throw new ArgumentOutOfRangeException(string0,
                    string.Format("{0} cannot set a count that exceed max count {1}", string0, byte_0.Length >> int3));
            }
            return num;
        }
    }
