...
namespace GHNamespaceG
{
    public class StreamClass1 : IDisposable
    {
            ...
            while (stream1.Position - num >= 16L && num2 < array.Length)
            {
                ...
                if (num3 < 0 || num3 > int0)
                {
                    Class355.Interface150.imethod_1(
                        string.Format("Found reference to channel {0}, but only {1} channels expected", num3, int0));
                }
                if ((array2[1] & 128) != 0)
                {
                    if (array[num3])
                    {
                        Class355.Interface150.imethod_0(
                            string.Format(
                                "Found end flag for channel {0}, but it is already marked as flagged.  This VGS may be corrupt.",
                                num3));
                    }
                    else
                    {
        ...
        public static void smethod_3(Stream stream1, int[] int0, int[] int1)
        {
            ...
            {
                Class355.Interface150.imethod_1(
                    string.Format("sampleCount length {0} does not match sampleRate length {1}", int0.Length,
                        int1.Length));
            }
            if (!smethod_0(stream1))
            {
                for (int i = 0; i < int0.Length; i++)
                {
                    if (int0[i] < 1)
                    {
                        throw new ApplicationException(
                            "Writing a VGS header requires knowing the data length ahead of time, or a stream that is capable of seeking and reading.");
                    }
                }
            }
